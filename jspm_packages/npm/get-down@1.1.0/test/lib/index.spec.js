...
(function(process) {
  ...
  describe('download()', function() {
    ...
    it('downloads a file', function(done) {
      var scope = mockHttp('http://example.com').get('/success.zip').replyWithFile(200, path.join(fixtures, 'success.zip'));
      download('http://example.com/success.zip').on('error', done).on('end', function(dest) {
    ...
    it('accepts an alternate dest directory', function(done) {
      ...
      download('http://example.com/success.zip', options).on('error', done).on('end', function(dest) {
    ...
    it('downloads and extracts a zip file', function(done) {
      ...
      download('http://example.com/success.zip', options).on('error', done).on('end', function(dest) {
        ...
        assert.equal(fs.readFileSync('dest/success.txt').toString(), 'success\n');
        done();
      });
    });
    it('extracts a zip file based on content-type', function(done) {
      var scope = mockHttp('http://example.com').get('/zipfiles/success').replyWithFile(200, path.join(fixtures, 'success.zip'), {'content-type': 'application/zip'});
      var options = {
        dest: 'dest',
        extract: true
      };
      download('http://example.com/zipfiles/success', options).on('error', done).on('end', function(dest) {
        assert.equal(path.resolve(dest), path.resolve('dest'));
        assert.isTrue(fs.existsSync('dest/success.txt'));
        assert.equal(fs.readFileSync('dest/success.txt').toString(), 'success\n');
        done();
