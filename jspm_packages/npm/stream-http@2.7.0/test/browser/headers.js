...
(function(Buffer) {
  ...
  test('arrays of headers', function(t) {
    http.get({
      ...
      headers: {'Test-Request-Header': ['foo', 'FOO2']}
    }, function(res) {
      var rawHeaders = [];
      for (var i = 0; i < res.rawHeaders.length; i += 2) {
        var lowerKey = res.rawHeaders[i].toLowerCase();
  ...
  var wrongMimeType = ((browserName === 'Chrome' && browserVersion <= 42) || ((browserName === 'Safari' || browserName === 'Mobile Safari') && browserVersion >= 6 && (browserVersion < 10 || (browserVersion === 10 && browserMinorVersion === 0))));
