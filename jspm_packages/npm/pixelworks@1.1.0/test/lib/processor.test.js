  ...
  describe('Processor', function() {
    ...
    describe('constructor', function() {
      it('creates a new processor', function() {
        var processor = new Processor({operation: identity});
        assert.instanceOf(processor, Processor);
      });
    });
    describe('#process()', function() {
      it('calls operation with input pixels', function(done) {
        var processor = new Processor({operation: function(inputs, meta) {
            ++meta.count;
            var pixel = inputs[0];
            for (var i = 0,
                ii = pixel.length; i < ii; ++i) {
              meta.sum += pixel[i];
            }
            return pixel;
          }});
      ...
      it('calls callback with processed image data', function(done) {
        var processor = new Processor({operation: function(inputs) {
            var pixel = inputs[0];
            pixel[0] *= 2;
            pixel[1] *= 2;
            pixel[2] *= 2;
        ...
        processor.process([input], {}, function(err, output, m) {
          ...
          assert.deepEqual(output.data, new Uint8ClampedArray([2, 4, 6, 8, 10, 12, 14, 16]));
          done();
        });
      });
      it('allows library functions to be called', function(done) {
        ...
        processor.process([input], {}, function(err, output, m) {
          ...
          assert.deepEqual(output.data, new Uint8ClampedArray([v0, v0, v0, 0, v1, v1, v1, 1]));
          done();
        });
      });
