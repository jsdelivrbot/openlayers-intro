  ...
  describe('Script', function() {
    ...
    describe('#provides', function() {
      ...
      it('identifies a valid base file', function(done) {
        scripts.read(path.join(fixtures, 'base', 'valid-base.js'), function(err, script) {
          if (err) {
            return done(err);
          }
      ...
      it('throws on invalid base file', function(done) {
        scripts.read(path.join(fixtures, 'base', 'invalid-base.js'), function(err, script) {
          if (err) {
            return done(err);
          }
    ...
    describe('#requires', function() {
      ...
      it('is an array of requires', function() {
        assert.deepEqual(script.requires, ['goog.asserts', 'goog.array']);
      });
    });
  });
  describe('read', function() {
    it('returns a promise that resolves to a script', function(done) {
      scripts.read(path.join(fixtures, 'basic', 'one.js'), function(err, script) {
        if (err) {
          return done(err);
        }
