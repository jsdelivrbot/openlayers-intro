    ...
    describe('#getDependencies()', function() {
      it('sorts lib scripts', function(done) {
        ...
        manager.on('ready', function() {
          ...
          assert.deepEqual(paths, ['base.js', 'food.js', 'fruit.js', 'banana.js']);
          done();
        });
      });
      it('provides dependencies for a main script (car)', function(done) {
        ...
        manager.on('error', done);
        manager.on('ready', function() {
          var dependencies = manager.getDependencies(path.join(fixtures, 'dependencies-main', 'main-car.js'));
          var paths = dependencies.map(function(s) {
            return path.basename(s.path);
          });
          assert.deepEqual(paths, ['base.js', 'fuel.js', 'vehicle.js', 'car.js', 'main-car.js']);
          done();
        });
      });
      it('provides dependencies for a main script (boat)', function(done) {
        ...
        manager.on('error', done);
        manager.on('ready', function() {
          var dependencies = manager.getDependencies(path.join(fixtures, 'dependencies-main', 'main-boat.js'));
          var paths = dependencies.map(function(s) {
            return path.basename(s.path);
          });
          assert.deepEqual(paths, ['base.js', 'fuel.js', 'vehicle.js', 'boat.js', 'main-boat.js']);
          done();
        });
      });
      it('does not provide main scripts if not requested', function(done) {
        ...
        manager.on('ready', function() {
          ...
          assert.deepEqual(paths.slice(0, 3), ['base.js', 'fuel.js', 'vehicle.js']);
      ...
      it('ignores files without requires or provides', function(done) {
        ...
        manager.on('ready', function() {
          ...
          assert.deepEqual(paths, ['base.js', 'parent.js', 'child.js']);
          done();
        });
      });
      it('includes scripts with goog.addDependency calls', function(done) {
        ...
        manager.on('error', done);
        manager.on('ready', function() {
          var dependencies = manager.getDependencies(path.join(fixtures, 'adds-deps', 'main.js'));
          var paths = dependencies.map(function(s) {
            return path.basename(s.path);
          });
          assert.deepEqual(paths, ['base.js', 'math.js', 'main.js', 'deps.js']);
          done();
        });
      });
      it('ignores requires when ignoreRequires matches', function(done) {
        ...
        manager.on('error', done);
        manager.on('ready', function() {
          var dependencies = manager.getDependencies(path.join(fixtures, 'dependencies-ignoreRequires', 'main.js'));
          var paths = dependencies.map(function(s) {
            return path.basename(s.path);
          });
          assert.deepEqual(paths, ['base.js', 'carrot.js', 'eggplant.js', 'main.js']);
          done();
        });
      });
    });
