...
(function(process) {
  ...
  exports = module.exports = function(options, callback) {
    ...
    var compilerJars = glob.sync(path.join(compilerDir, '*compiler*.jar'));
    if (compilerJars.length !== 1) {
      callback(new Error('No or more than one compiler found.'));
      return;
    }
    var args = options.jvm.concat('-jar', compilerJars[0]);
    var flagFile;
    if (options.compile) {
      var flags = [];
    ...
    child.on('close', function(code) {
      var err = null;
      if (code !== 0) {
        err = new Error('Process exited with non-zero status, ' + 'see log for more detail: ' + code);
      }
      if (flagFile) {
