...
var DEFAULT_OPTIONS = {
  language: 'en',
  resources: {en: JSON.parse(fs.readFileSync(path.join(__dirname, '../res/en.json'), 'utf8'))}
};
...
var TEMPLATE_VARIABLES = ['application', 'codeTagClose', 'codeTagOpen', 'element', 'field', 'functionNew', 'functionParams', 'functionReturns', 'functionThis', 'keyApplication', 'name', 'nullable', 'optional', 'param', 'prefix', 'repeatable', 'suffix', 'type'];
var FORMATS = {
  EXTENDED: 'extended',
  SIMPLE: 'simple'
};
...
Describer.prototype._stringify = function(type, typeString, useLongFormat) {
  var context = new Context({type: typeString || stringify(type, this._stringifyOptions)});
...
Describer.prototype._translate = function(key, context) {
  ...
  if (templateFunction === undefined) {
    throw new Error(util.format('The template %s does not exist for the language %s', key, this._options.language));
  }
  ...
  result = (templateFunction(_.extend(context, this._i18nContext)) || '').replace(/^\s+/, '');
...
Describer.prototype._modifierHelper = function(key, modifierPrefix, context) {
  ...
  return {
    extended: key ? this._translate(util.format('%s.%s.%s', modifierPrefix, FORMATS.EXTENDED, key), context) : '',
    simple: key ? this._translate(util.format('%s.%s.%s', modifierPrefix, FORMATS.SIMPLE, key), context) : ''
  };
};
...
function reduceMultiple(context, keyName, contextName, translate, previous, current, index, items) {
  var key = index === 0 ? '.first.many' : index === (items.length - 1) ? '.last.many' : '.middle.many';
  key = keyName + key;
...
Describer.prototype.nullable = function(nullable) {
  var key = nullable === true ? 'nullable' : nullable === false ? 'nonNullable' : '';
...
Describer.prototype._combineMultiple = function(items, context, keyName, contextName, useLongFormat) {
  var result = new Result();
  var self = this;
  var strings;
  strings = typeof items[0] === 'string' ? items.slice(0) : items.map(function(item) {
  ...
  switch (strings.length) {
    ...
    case 2:
      ...
      break;
    default:
      result.description = strings.reduce(reduceMultiple.bind(null, context, keyName, contextName, this._translate.bind(this)), '');
  }
...
Describer.prototype._addLinks = function(nameString) {
  ...
  if (options.links && Object.prototype.hasOwnProperty.call(options.links, nameString)) {
    ...
    nameString = util.format('<a href="%s"%s>%s</a>', options.links[nameString], linkClass, nameString);
  }
  return nameString;
};
