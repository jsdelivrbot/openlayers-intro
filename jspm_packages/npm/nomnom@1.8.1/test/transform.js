...
exports.testTransformCommand = function(test) {
  test.expect(1);
  var parser = nomnom().scriptName("test").options({addr: {transform: function(value) {
        var parts = value.split(":");
        return {
          host: parts[0],
          port: Number(parts[1])
  ...
  parser.command("run").options({string: {transform: function(value) {
        return value.toString();
