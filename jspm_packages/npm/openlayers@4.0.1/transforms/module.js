...
module.exports = function(info, api) {
  ...
  const replacements = {};
  root.find(j.MemberExpression, defineMemberExpression).filter((path) => {
  ...
  let provide;
  root.find(j.ExpressionStatement, getGoogExpressionStatement('provide')).forEach((path) => {
    if (provide) {
  ...
  root.find(j.ExpressionStatement, getMemberExpressionAssignment(provide)).replaceWith((path) => {
    declaredProvide = true;
    const statement = j.variableDeclaration('var', [j.variableDeclarator(j.identifier(rename(provide)), path.value.expression.right)]);
    statement.comments = path.value.comments;
    return statement;
  });
  if (!declaredProvide) {
    const body = root.find(j.Program).get('body');
    body.unshift(j.variableDeclaration('var', [j.variableDeclarator(j.identifier(rename(provide)), j.objectExpression([]))]));
  }
  const imports = [];
  root.find(j.ExpressionStatement, getGoogExpressionStatement('require')).forEach((path) => {
    ...
    imports.push(j.importDeclaration([j.importDefaultSpecifier(j.identifier(renamed))], j.literal(resolve(provide, name))));
  }).remove();
  const body = root.find(j.Program).get('body');
  body.unshift.apply(body, imports);
  Object.keys(replacements).sort().reverse().forEach((name) => {
    if (name.indexOf('.') > 0) {
      root.find(j.MemberExpression, getMemberExpression(name)).replaceWith(j.identifier(replacements[name]));
    } else {
      root.find(j.Identifier, {name: name}).replaceWith(j.identifier(replacements[name]));
    }
  });
  root.find(j.Program).get('body').push(j.exportDefaultDeclaration(j.identifier(rename(provide))));
