...
test('identical content gets deduped and the row gets an implicit dep on the original source', function(t) {
  t.plan(1);
  var rows = [];
  browserify().on('dep', [].push.bind(rows)).require(require.resolve('./dup'), {entry: true}).bundle(check);
...
test('identical content gets deduped with fullPaths', function(t) {
  t.plan(1);
  var rows = [];
  browserify({fullPaths: true}).on('dep', [].push.bind(rows)).require(require.resolve('./dup'), {entry: true}).bundle(check);
  function check(err, src) {
    if (err)
      return t.fail(err);
    var deduped = rows.filter(function(x) {
...
test('identical content does not get deduped with dedupe option false', function(t) {
  ...
  browserify({
    fullPaths: true,
    dedupe: false
  }).on('dep', [].push.bind(rows)).require(require.resolve('./dup'), {entry: true}).bundle(check);
  function check(err, src) {
    if (err)
      return t.fail(err);
    var deduped = rows.filter(function(x) {
